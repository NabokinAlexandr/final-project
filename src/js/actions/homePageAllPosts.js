...
function getAllUsersPosts (users, currentUser) {
...
function addToDom(template, currentUser) {
...
function homePageAllPosts(users, currentUser) {
    if (!currentUser) {
        const api = new Apis();
        api.getCurrentUser()
        .then(resp => {
            if (resp.data.length > 0) {
                ...
                getAllUsersPosts(users, currentUser)
                .then(
                    result => addToDom(result, currentUser),
